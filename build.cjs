...
const articles = require('./src/data/articles.cjs');
const articleGenerator = require('./src/data/articleGenerator.cjs');
...
const airports = [
  {
    ...
    features: ["IEPL专线", "Trojan", "Shadowsocks"],
    nodeTypes: ["IEPL", "Trojan", "Shadowsocks"],
    streaming: ["Netflix", "YouTube"],
    desc: "高性价比 IEPL 专线，8元/月起全节点1倍率不限速，轻松解锁 Netflix、YouTube Premium 及 ChatGPT AI 工具。",
  ...
  },
  {
    ...
    features: ["IPLC专线", "Shadowsocks", "V2Ray"],
    nodeTypes: ["IPLC", "Shadowsocks", "V2Ray"],
    streaming: ["Netflix", "YouTube"],
    desc: "全线 IPLC 专线，7.42元/月起高峰期满速不降速，自有机房独立过境，低延迟无丢包体验。",
  ...
  },
  {
    ...
    price: "1.00",
    rating: "9.4",
    slug: "kunpeng-jiasu-review",
    features: ["专线中转", "原生节点", "Trojan"],
    nodeTypes: ["原生节点", "Trojan", "Shadowsocks"],
    streaming: ["Netflix", "ChatGPT", "Disney+"],
    desc: "全球多国原生节点与冷门地区线路，全站节点统一 1.0 倍率扣费无暗扣。完美解锁 ChatGPT 与海外流媒体。",
    affLink: "https://kunpengjiasu.com/#/register?code=QAo8sMhc",
    rank: 3,
    noLimit: true,
    hasTrial: true,
    tgGroup: "",
  ...
  },
  {
    ...
    rating: "9.3",
    slug: "flycat-review",
    features: ["IPLC专线", "原生IP", "Trojan"],
    nodeTypes: ["IPLC", "Shadowsocks", "Trojan"],
    streaming: ["Netflix", "Disney+"],
    desc: "全 IPLC 专线网络，最高 2.5Gbps 稳定速率不限设备，提供纯净原生 IP 解锁大模型与影音。",
...
function build() {
  ...
  const airportCardsMarkup = airports.map(ap => {
    const rating = ap.rating || (10 - (ap.rank - 1) * 0.1).toFixed(1);
    const nodePills = (ap.nodeTypes || ap.features.slice(0, 2)).map(f => `<span style="background-color:#e0f2fe;color:#0369a1;font-size:0.7rem;padding:0.2rem 0.6rem;border-radius:9999px;font-weight:600;border:1px solid #bae6fd;display:inline-block;">${f}</span>`).join(' ');
    const streamPills = (ap.streaming || ['Netflix', 'YouTube']).slice(0, 2).map(s => `<span style="background-color:#d1fae5;color:#047857;font-size:0.7rem;padding:0.2rem 0.6rem;border-radius:9999px;font-weight:600;border:1px solid #a7f3d0;display:inline-block;">${s}</span>`).join(' ');
    return `
      <div class="airport-card" style="border: 2px solid #fcd34d; border-radius: 16px; background-color: #ffffff; padding: 1.25rem; box-shadow: 0 4px 15px rgba(251, 191, 36, 0.08); display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
            <div>
              <span style="background-color: #fef3c7; color: #b45309; font-size: 0.75rem; font-weight: 700; padding: 0.15rem 0.6rem; border-radius: 9999px; display: inline-block; margin-bottom: 0.3rem;">推荐</span>
              <h3 style="font-size: 1.2rem; font-weight: 900; color: #0f172a; margin: 0; line-height: 1.2;">${ap.name}</h3>
            </div>
            <div style="background-color: #f8fafc; border: 1px solid #f1f5f9; border-radius: 12px; padding: 0.25rem 0.6rem; text-align: center; min-width: 48px;">
              <span style="font-size: 1.3rem; font-weight: 900; color: #f59e0b; line-height: 1;">${rating}</span>
              <span style="font-size: 0.65rem; color: #94a3b8; display: block;">/ 10</span>
            </div>
          </div>
          <p style="font-size: 0.8rem; color: #64748b; margin: 0.4rem 0 0.6rem; line-height: 1.45; min-height: 2.6rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${ap.desc}</p>
          <div style="color: #f59e0b; font-size: 0.85rem; margin-bottom: 0.75rem; tracking: -1px;">★★★★★</div>
          <div style="display: flex; flex-wrap: wrap; gap: 0.35rem; margin-bottom: 1rem;">
            ${nodePills}
            ${streamPills}
          </div>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 0.75rem; border-top: 1px solid #f1f5f9; margin-top: auto;">
          <div style="font-size: 0.8rem; color: #64748b;">
            低至 <strong style="font-size: 1.05rem; font-weight: 900; color: #0f172a;">￥${ap.price}</strong>/${ap.price === "1.00" ? "次" : "月"}
          </div>
          <div style="display: flex; gap: 0.5rem; align-items: center;">
            <a href="posts/${ap.slug}.html" style="font-size: 0.8rem; font-weight: 600; color: #475569; text-decoration: none;">查看评测 →</a>
            <a href="${ap.affLink}" target="_blank" rel="nofollow" style="background-color: #f59e0b; color: #ffffff; font-size: 0.8rem; font-weight: 800; padding: 0.35rem 1rem; border-radius: 9999px; text-decoration: none; box-shadow: 0 2px 8px rgba(245, 158, 11, 0.3);">官网</a>
          </div>
        </div>
      </div>
    `;
    ...
    return `
      <div class="airport-card" style="border: 2px solid #fcd34d; border-radius: 16px; background-color: #ffffff; padding: 1.25rem; box-shadow: 0 4px 15px rgba(251, 191, 36, 0.08); display: flex; flex-direction: column; justify-content: space-between;">
        <div>
          ...
          <p style="font-size: 0.8rem; color: #64748b; margin: 0.4rem 0 0.6rem; line-height: 1.45; min-height: 2.6rem; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${ap.desc}</p>
          <div style="color: #f59e0b; font-size: 0.85rem; margin-bottom: 0.75rem;">★★★★★</div>
          <div style="display: flex; flex-wrap: wrap; gap: 0.35rem; margin-bottom: 1rem;">
            ${nodePills}
            ${streamPills}
          </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 0.75rem; border-top: 1px solid #f1f5f9; margin-top: auto;">
          ...
          <div style="display: flex; gap: 0.5rem; align-items: center;">
            <a href="${ap.slug}.html" style="font-size: 0.8rem; font-weight: 600; color: #475569; text-decoration: none;">查看评测 →</a>
            <a href="${ap.affLink}" target="_blank" rel="nofollow" style="background-color: #f59e0b; color: #ffffff; font-size: 0.8rem; font-weight: 800; padding: 0.35rem 1rem; border-radius: 9999px; text-decoration: none; box-shadow: 0 2px 8px rgba(245, 158, 11, 0.3);">官网</a>
          </div>
        </div>
      </div>
